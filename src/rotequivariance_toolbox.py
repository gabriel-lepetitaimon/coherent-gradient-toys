...
def cartesian_space(size, center=None, device=None):
    ...
    x = torch.linspace(-center[0], size[0] - center[0], size[0], device=device)
    y = torch.linspace(-center[1], size[1] - center[1], size[1], device=device)
    x, y = torch.meshgrid(x, y)
    return y, x


def r_space(size, center=None):
    return torch.linalg.norm(cartesian_space(size, center=center), axis=0)


def polar_space(size, center=None):
    x, y = cartesian_space(size, center=center)
    rho = torch.linalg.norm((x, y), axis=0)
    phi = torch.atan2(y, x)
    return rho, phi
...
class SpectralPower:
    ...
    def spectral_power(signal: torch.Tensor, mask: Union[str, torch.Tensor] = None, merged_dim=None):
        ...
        if merged_dim is None:
            merged_dim = tuple(range(max(-2, signal.dim-1), 0))

        if mask == 'disk':
            r = min(signal.shape[-2:])/2
            mask = (r_space(signal.shape[-2:])) <= r
        elif mask == 'diamond':
    ...
    def polar_spectral_power(self, signal):
        return SpectralPower.spectral_power(signal)
